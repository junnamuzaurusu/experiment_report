...
void bandpass(complex double * y, long n, long low, long high) {
  int i;
  long freq;
  for (i = 0; i < n; i++) {
    freq = i * SAMPLE_RATE / n;
    if (freq >= SAMPLE_RATE / 2) freq = freq - SAMPLE_RATE;
    if (labs(freq) < low || labs(freq) > high) y[i] = 0;
  }
}
